...
def feat_job(list_, folder1, folder2):
    ...
    for ilist in list_:
        ...
        fp.write(f'cd /mnt/research/guowei-search.4/junjie-ppi/4EEF/{dst}\n')
...
def gen_PDBs(PDBid, Antibody, Antigen, Chain, resWT, resID, resMT, pH):
    ...
    os.system("ln -s ../../4EEF.pdb 4EEF.pdb")
...
if not os.path.exists("./4EEF/features/"):
    os.system("mkdir ./4EEF/features/")

seq_list = dataset_list("./4EEF/4EEF.txt")
#print(seq_list)
...
#for i in range(len(seq_list)):
    #ilist = seq_list[i]
    #PDBid, Antibody, Antigen, Chain, resWT, resID, resMT, pH = ilist[0], ilist[1], ilist[2], ilist[3], ilist[4], ilist[5], ilist[6], ilist[7]
    #gen_PDBs(PDBid, Antibody, Antigen, Chain, resWT, resID, resMT, pH)
#os.chdir("../")

if not os.path.exists("./4EEF/blast_jobs/"):
    os.mkdir("./4EEF/blast_jobs/")
if not os.path.exists("./4EEF/blast_out/"):
    os.mkdir("./4EEF/blast_out/")
...
if not os.path.exists("./4EEF/feat_jobs/"):
    os.mkdir("./4EEF/feat_jobs/")

if not os.path.exists("./4EEF/feat_out/"):
    os.mkdir("./4EEF/feat_out/")

feat_job(seq_list, "features", "feat_jobs")
run_feat(seq_list, "features", "feat_jobs")

if not os.path.exists("./4EEF/seq_jobs/"):
    os.mkdir("./4EEF/seq_jobs/")

if not os.path.exists("./4EEF/seq_out/"):
    os.mkdir("./4EEF/seq_out/")
