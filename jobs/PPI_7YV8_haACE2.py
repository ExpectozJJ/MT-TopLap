...
def feat_job(list_, folder1, folder2):
    ...
    for ilist in list_:
        ...
        fp.write(f'cd /mnt/research/guowei-search.4/junjie-ppi/7YV8_haACE2/{dst}\n')
...
def gen_PDBs(PDBid, Antibody, Antigen, Chain, resWT, resID, resMT, pH):
    ...
    #os.system("ln -s 7YV8_haACE2/{}.pdb {}{}.pdb".format(PDBid, curr_dir, PDBid))
...
if not os.path.exists("./7YV8_haACE2/features/"):
    os.system("mkdir ./7YV8_haACE2/features/")

seq_list = dataset_list("./7YV8_haACE2/7YV8_RBD.txt")
#print(seq_list)
#mp_gen_PDBs(seq_list)

if not os.path.exists("./7YV8_haACE2/blast_jobs/"):
    os.mkdir("./7YV8_haACE2/blast_jobs/")
if not os.path.exists("./7YV8_haACE2/blast_out/"):
    os.mkdir("./7YV8_haACE2/blast_out/")
...
if not os.path.exists("./7YV8_haACE2/feat_jobs/"):
    os.mkdir("./7YV8_haACE2/feat_jobs/")

if not os.path.exists("./7YV8_haACE2/feat_out/"):
    os.mkdir("./7YV8_haACE2/feat_out/")

#feat_job(seq_list, "features", "feat_jobs")
#run_feat(seq_list, "features", "feat_jobs")

if not os.path.exists("./7YV8_haACE2/seq_jobs/"):
    os.mkdir("./7YV8_haACE2/seq_jobs/")

if not os.path.exists("./7YV8_haACE2/seq_out/"):
    os.mkdir("./7YV8_haACE2/seq_out/")
...
structure = PDBParser().get_structure('7yv8', './7YV8_haACE2/7YV8.pdb')    
...
for i in chain.get_residues():
    if i.get_resname() in d.keys():
        ...
        for j in rows:
            if j != res_code:
                if not os.path.exists(f'./7YV8_haACE2/features/7YV8_B_{res_code}_{res_id}_{j}/'):
...
for i in range(len(rows)):
    for j in range(len(missing_id)):
        if missing_id[j][0] != rows[i]
            new_list.append(['7YV8', 'A', 'B', 'B', missing_id[j][0], missing_id[j][1:], rows[i], '7.0'])
