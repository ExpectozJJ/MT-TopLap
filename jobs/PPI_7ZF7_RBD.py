...
def feat_job(list_, folder1, folder2):
    ...
    for ilist in list_:
        ...
        fp.write(f'cd /mnt/research/guowei-search.4/junjie-ppi/7ZF7_RBD/{dst}\n')
...
def gen_PDBs(PDBid, Antibody, Antigen, Chain, resWT, resID, resMT, pH):
    ...
    #os.system("ln -s 7ZF7_RBD/{}.pdb {}{}.pdb".format(PDBid, curr_dir, PDBid))
...
if not os.path.exists("./7ZF7_RBD/features/"):
    os.system("mkdir ./7ZF7_RBD/features/")

seq_list = dataset_list("./7ZF7_RBD/7ZF7_RBD.txt")
#print(seq_list)
#mp_gen_PDBs(seq_list)

if not os.path.exists("./7ZF7_RBD/blast_jobs/"):
    os.mkdir("./7ZF7_RBD/blast_jobs/")
if not os.path.exists("./7ZF7_RBD/blast_out/"):
    os.mkdir("./7ZF7_RBD/blast_out/")
...
if not os.path.exists("./7ZF7_RBD/feat_jobs/"):
    os.mkdir("./7ZF7_RBD/feat_jobs/")

if not os.path.exists("./7ZF7_RBD/feat_out/"):
    os.mkdir("./7ZF7_RBD/feat_out/")

#feat_job(seq_list, "features", "feat_jobs")
#run_feat(seq_list, "features", "feat_jobs")

if not os.path.exists("./7ZF7_RBD/seq_jobs/"):
    os.mkdir("./7ZF7_RBD/seq_jobs/")

if not os.path.exists("./7ZF7_RBD/seq_out/"):
    os.mkdir("./7ZF7_RBD/seq_out/")
...
structure = PDBParser().get_structure('7zf7', './7ZF7_RBD/7ZF7.pdb')    
...
for i in chain.get_residues():
    if i.get_resname() in d.keys():
        ...
        for j in rows:
            if j != res_code:
                if not os.path.exists(f'./7ZF7_RBD/features/7ZF7_B_{res_code}_{res_id}_{j}/') \
                    or not os.path.exists(f'./7ZF7_RBD/features/7ZF7_B_{res_code}_{res_id}_{j}/7ZF7_B_{res_code}_{res_id}_{j}_seq.npy') \
                    or not os.path.exists(f'./7ZF7_RBD/features/7ZF7_B_{res_code}_{res_id}_{j}/7ZF7_B_{j}_{res_id}_{res_code}_seq.npy'):
...
for j in range(len(missing_id)):
    ...
    new_list.append(['7ZF7', 'A', 'B', 'B', missing_id[j][0], missing_id[j][1:-1], missing_id[j][-1], '7.0'])
