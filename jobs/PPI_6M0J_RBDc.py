...
def feat_job(list_, folder1, folder2):
    ...
    for ilist in list_:
        ...
        fp.write(f'cd /mnt/research/guowei-search.4/junjie-ppi/6M0J_RBDc/{dst}\n')
...
def create_blastjob(list_, folder1, folder2):
    ...
    for ilist in list_:
        ...
        fp.write('python PPIprepare.py '+' '.join(ilist[:-1])+'\n')
        #fp.write('python PPIfeature_Lap.py '+' '.join(ilist[:-1])+'\n')
        fp.close()
    os.chdir("../")
    return
...
def seq_job(list_, folder1, folder2):
    ...
    for ilist in list_:
        ...
        fp.write(f'cd /mnt/research/guowei-search.4/junjie-ppi/6M0J_RBDc/{dst}\n')
        #fp.write('source activate pytorch\n')
        fp.write('python PPIfeature_seq.py '+' '.join(ilist[:-1])+'\n')
        fp.close()
    os.chdir("../")
    return
...
def gen_PDBs(PDBid, Antibody, Antigen, Chain, resWT, resID, resMT, pH):
    ...
    #os.system("ln -s 6M0J_RBDc/{}.pdb {}{}.pdb".format(PDBid, curr_dir, PDBid))
...
if not os.path.exists("./6M0J_RBDc/features/"):
    os.system("mkdir ./6M0J_RBDc/features/")

seq_list = dataset_list("./6M0J_RBDc/6M0J_RBDc.txt")
#print(seq_list)
#mp_gen_PDBs(seq_list)

if not os.path.exists("./6M0J_RBDc/blast_jobs/"):
    os.mkdir("./6M0J_RBDc/blast_jobs/")
if not os.path.exists("./6M0J_RBDc/blast_out/"):
    os.mkdir("./6M0J_RBDc/blast_out/")
...
if not os.path.exists("./6M0J_RBDc/feat_jobs/"):
    os.mkdir("./6M0J_RBDc/feat_jobs/")

if not os.path.exists("./6M0J_RBDc/feat_out/"):
    os.mkdir("./6M0J_RBDc/feat_out/")

feat_job(seq_list, "features", "feat_jobs")
#run_feat(seq_list, "features", "feat_jobs")

if not os.path.exists("./6M0J_RBDc/seq_jobs/"):
    os.mkdir("./6M0J_RBDc/seq_jobs/")

if not os.path.exists("./6M0J_RBDc/seq_out/"):
    os.mkdir("./6M0J_RBDc/seq_out/")
