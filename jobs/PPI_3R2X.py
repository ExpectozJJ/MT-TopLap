...
def feat_job(list_, folder1, folder2):
    ...
    for ilist in list_:
        ...
        fp.write(f'cd /mnt/research/guowei-search.4/junjie-ppi/3R2X/{dst}\n')
...
def gen_PDBs(PDBid, Antibody, Antigen, Chain, resWT, resID, resMT, pH):
    ...
    os.system("ln -s ../../3R2X.pdb 3R2X.pdb")
...
if not os.path.exists("./3R2X/features/"):
    os.system("mkdir ./3R2X/features/")

seq_list = dataset_list("./3R2X/3R2X.txt")
#print(seq_list)
#mp_gen_PDBs(seq_list)

if not os.path.exists("./3R2X/blast_jobs/"):
    os.mkdir("./3R2X/blast_jobs/")
if not os.path.exists("./3R2X/blast_out/"):
    os.mkdir("./3R2X/blast_out/")
...
if not os.path.exists("./3R2X/feat_jobs/"):
    os.mkdir("./3R2X/feat_jobs/")

if not os.path.exists("./3R2X/feat_out/"):
    os.mkdir("./3R2X/feat_out/")

feat_job(seq_list, "features", "feat_jobs")
#run_feat(seq_list, "features", "feat_jobs")

if not os.path.exists("./3R2X/seq_jobs/"):
    os.mkdir("./3R2X/seq_jobs/")

if not os.path.exists("./3R2X/seq_out/"):
    os.mkdir("./3R2X/seq_out/")
